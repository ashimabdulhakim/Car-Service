...
<?php
	...
	$query = 'SELECT DISTINCT field FROM kondisi_kaki';
	// Execute the query
	...
	if (isset($_POST["submitKaki"])){
		...
		if($valid){
			...
			$query2 = "SELECT * FROM kondisi_kaki where field = 'goal'";
			// Execute the query
			$result2 = $db->query( $query2 );
			$j = 1;
			while($goal = $result2->fetch_object()){
				$kondisi[$j] = $goal->kondisi;
				$query3 = "SELECT * FROM kaki where goal = '".$kondisi[$j]."'";
				// Execute the query
				...
				for( $i= 1 ; $i <= $jumfield ; $i++ ){
					$query4 = "SELECT * FROM kaki where F".$i." = '".$N[$i]."' AND goal = '".$kondisi[$j]."'";
					// Execute the query
			...
			<div class="row team-bottom">
				<div class="col-sm-4 team-grid">
					...
					<div class="col-sm-4 team-grid mt-sm-0 mt-3">
						...
						<table align="center" style="margin-left: 45px;">
							<?php
								for( $i= 1 ; $i <= $jumfield ; $i++ ){
									echo '<tr>';
									echo '<td>';
									echo '<div class="form-group">';
										//Asign query field
										$queryfield = " SELECT * FROM kondisi_kaki WHERE field='F".$i."' ";
										// Execute the query
							...
							<button type="submit" name="submitKaki" class="btn btn-secondary">Submit</button>
